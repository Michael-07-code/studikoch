    ...
    <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {items.map((s) => (
        <Card key={s.recipe.id} className="p-2 text-sm" padded={false}>
          <p className="font-medium text-stone-900">{s.recipe.name}</p>
    ...
    <div className="space-y-6">
        ...
        {period === 'tag' ? (
          <label className="flex flex-col text-sm text-stone-700">
            ...
            <input
              ...
              className="mt-1 w-32 rounded-xl border border-stone-300 px-2 py-1.5 text-sm"
            />
          </label>
        ) : (
          <label className="flex flex-col text-sm text-stone-700">
      ...
      {settings.splitByMeal && dailyBase !== undefined && (
        <div className="flex flex-wrap gap-4">
          {MEAL_TYPES.map((type) => (
            <label key={type} className="flex flex-col text-xs text-stone-600">
              ...
              <input
                ...
                className="mt-1 w-24 rounded-xl border border-stone-300 px-2 py-1"
              />
            </label>
          ))}
        </div>
